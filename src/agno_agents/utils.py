# utils.py - Utility functions and tools for DSPy implementation
...
def extract_metrics(response):
    """Extract metrics from DSPy response objects"""
...
class ValuationTools(Toolkit):
    ...
    def revenue_multiple_valuation(self, revenue: float, multiple: Union[float, int] = 2) -> float:
        """Calculate valuation using revenue multiple method"""
        valuation = revenue * multiple
        return valuation

    def profit_multiple_valuation(self, profit: float, multiple: Union[float, int] = 8) -> float:
        """Calculate valuation using profit multiple method"""
